...
export default function EditLectureInputs({lectureData, conferenceData, currentUserData}: params) {

    const [name, setName] = useState<string>(lectureData.name);
    const [description, setDescription] = useState<string>(lectureData.description);
    const [startDateTime, setStartDateTime] = useState<string>(lectureData.startDateTime);
    const [durationMinutes, setDurationMinutes] = useState<string>(lectureData.durationMinutes.toString());
    const [imageId, setImageId] = useState<number>(lectureData.image.id);
    const [imageFile, setImageFile] = useState<File>(new File([], ""));
    const [lecturersIds, setLecturersIds] = useState<number[]>(lectureData.lecturers.map(lecturer => lecturer.id));
    ...
    const handleDeleteLecturers = (indexToDelete: number) => {
        ...
        setLecturersUsernames(lecturersUsernames.filter((_, index) => index !== indexToDelete));
    };
    ...
    const handleEditLectureLecturer = async () => {
        ...
        try {
            const result = await modifyLectureInfoByLecturer(lectureData.id, {description, imageId});
            if (result !== "Brak autoryzacji użytkownika" && result !== "Nie jesteś prelegentem" && result !== "Wystąpił błąd podczas modyfikacji danych prelekcji") {
                setName("");
                setDescription("");
                const form = document.getElementById("imageInput") as HTMLFormElement;
                if (form) {
    ...
    return (
        <div className="flex flex-col space-y-6 text-darkblue">
            {currentUserData.role === "ADMIN" || currentUserData.role === "ORGANIZER" &&
            ...
            <div className="flex flex-col items-center justify-center w-full">
                <button
                    onClick={currentUserData.role !== 'USER' ? handleEditLectureOrganizer : handleEditLectureLecturer}
