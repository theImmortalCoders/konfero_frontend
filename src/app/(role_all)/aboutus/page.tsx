...
function Header({children}: { children: React.ReactNode }) {
    return (
        <h1 className="text-xl md:text-2xl font-black mb-4 text-center">
            {children}
        </h1>
    );
}
...
function Description({children}: { children: React.ReactNode }) {
    return (
        <p className="text-center text-sm lg:text-base lg:text-justify">
            {children}
        </p>
    );
}
...
function AppDescription() {
    return (
        <Box className="w-[90%] lg:w-[75%] xl:w-[60%] text-darkblue">
            ...
            <Description>
                Konfero to innowacyjne narzędzie, które rewolucjonizuje sposób
                organizacji konferencji naukowych. Nasza aplikacja webowa została
                zaprojektowana z myślą o potrzebach organizatorów konferencji, aby
                wspierać ich w każdym etapie przygotowań i realizacji wydarzenia.
            </Description>
        </Box>
    );
}
...
function UserOpinion() {
    ...
    return (
        <Box
            className="flex flex-col md:flex-row items-center w-[90%] lg:w-[75%] xl:w-[60%] text-darkblue space-x-0 md:space-x-8 lg:mr-[25%]">
            ...
            <div className="md:w-3/5 order-1 md:order-2">
                ...
                <Description>
                    Nasza aplikacja jest idealna dla organizatorów konferencji naukowych
                    wszelkich rozmiarów. Niezależnie od tego, czy jest to małe sympozjum,
                    czy międzynarodowy kongres, Konfero zapewnia narzędzia niezbędne do
                    stworzenia udanego wydarzenia.
                </Description>
            </div>
        </Box>
    );
...
function Cooperation() {
    return (
        <Box
            className="flex flex-col md:flex-row items-center w-[90%] lg:w-[75%] xl:w-[60%] text-darkblue space-x-0 md:space-x-8 lg:ml-[25%]">
            <div className="w-5/6 lg:w-4/5">
                <Header>Współpraca i rozwój</Header>
                <Description>
                    Wierzymy w moc współpracy i otwartego dialogu. Nasz zespół regularnie
                    uczestniczy w konferencjach branżowych, warsztatach i szkoleniach, aby
                    być na bieżąco z najnowszymi trendami i najlepszymi praktykami w
                    branży IT. To pozwala nam nie tylko rozwijać nasze umiejętności, ale
                    także dzielić się wiedzą i doświadczeniem z całą społecznością.
                </Description>
            </div>
            <Image
                src={Photo_2}
                alt="Photo_2"
                className="relative xl:left-16 w-5/6 md:w-2/5 mt-5 lg:mt-0 rounded-3xl"
            />
        </Box>
    );
}
...
function Picture() {
    return <FaUserTie className="text-6xl md:text-7xl"/>;
}

function TeamMember({children}: { children: React.ReactNode }) {
    return <p className="text-xs md:text-sm text-nowrap">{children}</p>;
}

function Profession({children}: { children: React.ReactNode }) {
    return <p className="text-xs font-sans text-nowrap">{children}</p>;
}
    ...
    return (
        <Box className="w-[90%] lg:w-[75%] xl:w-[60%] text-darkblue pb-0 sm:pb-0">
            ...
            <div className="grid grid-cols-4 md:grid-cols-5 justify-items-center items-center gap-y-10">
                ...
                <div
                    className="flex flex-col col-span-4 md:col-span-3 justify-center items-center space-y-5 px-4 pt-4 pb-8 border-2 border-b-0 border-darkblue rounded-t-3xl order-8 md:order-7">
                    <p className="text-sm sm:text-lg text-nowrap">
