...
export async function deleteMaterialFromLecture(
  lectureId: number,
  materialId: number,
) {
  try {
...
export async function addMaterialToLecture(
  lectureId: number,
  materialId: number,
) {
  try {
    ...
    if (response.status === 200) {
      ...
      console.error("Wystąpił błąd podczas dodawania materiału do prelekcji");
